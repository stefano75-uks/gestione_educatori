...
if (isset($_POST['upload'])) {
    ...
    $tempFilePath = $tempDir . DIRECTORY_SEPARATOR . uniqid() . '-' . $_FILES['file']['name'];
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], $tempFilePath)) {
        ...
        } elseif ($fileType === 'text/csv' || $fileType === 'application/csv' || $fileType === 'application/octet-stream') {
            // File CSV
            $csv = Reader::createFromPath($tempFilePath, 'r');
            $csv->setHeaderOffset(0); // Prima riga come intestazioni
            $fields = $csv->getHeader(); // Ottieni i campi dall'intestazione
        } else {
            die("Tipo di file non supportato.");
        }
    } else {
    ...
    echo '<input type="hidden" name="tempFilePath" value="' . htmlentities($tempFilePath) . '">'; // Salva il percorso del file temporaneo
    echo '<div class="row">';
    ...
    foreach ($fields as $index => $field) {
        ...
        echo '<select name="fields[' . htmlentities($field) . ']" class="form-control">';
        echo '<option value="">--Seleziona--</option>';
        foreach ($tableFields as $tableField) {
            echo '<option value="' . htmlentities($tableField) . '">' . ucfirst($tableField) . '</option>';
        }
        echo '</select>';
        echo '</div>';
        echo '</div>';
    ...
    if (empty($tempFilePath) || empty($fields)) {
    ...
    if (file_exists($tempFilePath)) {
        if (mime_content_type($tempFilePath) === 'application/vnd.ms-excel' || mime_content_type($tempFilePath) === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
            // File Excel
            $spreadsheet = IOFactory::load($tempFilePath);
            $worksheet = $spreadsheet->getActiveSheet();
            $rows = $worksheet->toArray();
            array_shift($rows); // Rimuovi la prima riga (intestazioni)
        } elseif (mime_content_type($tempFilePath) === 'text/csv' || mime_content_type($tempFilePath) === 'application/csv' || mime_content_type($tempFilePath) === 'application/octet-stream') {
            // File CSV
            $csv = Reader::createFromPath($tempFilePath, 'r');
            $csv->setHeaderOffset(0); // Prima riga come intestazioni
            $rows = iterator_to_array($csv->getRecords()); // Ottieni i record
        } else {
            die("Tipo di file non supportato.");
        }
    } else {
        die("File non valido o non selezionato.");
    }
    ...
    foreach ($rows as $row) {
        ...
        foreach ($fields as $excelField => $dbField) {
            if (!empty($dbField)) {
                $data[$dbField] = $row[$excelField] ?? null;
            }
        }
    ...
    echo '<form action="import.php" method="post" enctype="multipart/form-data">';
    echo '<div class="form-group">';
    echo '<label for="file">Seleziona File Excel o CSV:</label>';
